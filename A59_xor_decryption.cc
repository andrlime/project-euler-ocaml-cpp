...
decrypt(std::string& s, const std::vector<char>& cipher, const std::array<char, 3> key)
{
    for (std::size_t index = 0; index < cipher.size(); index++) {
        ...
        const bool is_reasonable_punctuation = next_char == ',' || next_char == '.'
                                               || next_char == ';' || next_char == ':';
        const bool is_other_punctuation = next_char == '!' || next_char == '-'
                                          || next_char == '?' || next_char == '\''
                                          || next_char == '\"';
        ...
        if (is_lowercase || is_uppercase || is_digit || is_space
            || is_reasonable_punctuation || is_other_punctuation || is_parenthesis
            || is_bracket || is_math) {
