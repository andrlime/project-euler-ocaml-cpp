...
let create_units (n : int) : units =
  if n = 0 then Zero
    ...
    if n < 20 then Onety(n)
    else Multy(n/10, n mod 10)
  )
;;
...
let rec count_letters_in_number (n : int) : int =
  ...
  | Hundred h, Multy (t, o) -> count_letters_in_number h + 7 + 3 + count_letters_in_number (t * 10 + o)
  ...
  | Zero, Multy (t, o) -> (
    let tens_digit_count = match t with
    | 9 -> 6
    | 8 -> 6
    | 7 -> 7
    | 6 -> 5
